...
public class EpiTabGridDynamics extends EpiTabDefinitions {
	...
	private void updateCellDivisionPanel() {
		...
		jspHeritableComponents.setPreferredSize(
				new Dimension(Math.max(170, jpHeritableComponents.getWidth()), Math.min(200, i * 50)));
		...
		jtf.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				try {
					setExpression(jtf.getText(), CellularEvent.PROLIFERATION);
	...
	private void updateCellDeathPanel() {
		...
		jtf.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				JTextField jtf = (JTextField) e.getSource();
				try {
					setExpression(jtf.getText(), CellularEvent.APOPTOSIS);
					jtf.setBackground(Color.WHITE);
				} catch (RecognitionException re) {
					jtf.setBackground(ColorUtils.LIGHT_RED);
				} catch (RuntimeException re) {
	...
	private void setExpression(String expression, CellularEvent eventType)
		...
		this.eventManager.setModelEventExpression(m, eventType, new ModelEventExpression(expression));
	}
	...
	public void applyChange() {
		...
		for (LogicalModel m : modelList) {
			for (CellularEvent event : this.eventManager.getModelEvents(m)) {
				ModelEventExpression meExpr = this.eventManager.getModelEventExpression(m, event);
				try {
					this.epithelium.getModelEventManager().setModelEventExpression(m, event, meExpr);
				} catch (RecognitionException re) {
